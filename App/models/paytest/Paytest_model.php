	...
	*/
	...
	public function invest($body) {
		...
		if($respCode === '000000') {
			...
			// 添加用户首投,2019-2-13, 3-1逻辑修改为97天首投记录,3-28号逻辑修改，判断不出首投，因为新手标投过
			if($info['borrow_duration'] == 4 && $info['borrow_type'] !=5) {
				$count = $this->db->where(array('investor_uid'=>$investor_uid,'first'=>1))->count_all_results('borrow_investor');
				if(!$count) {
					$data['investor']['first'] = 1;
				}
			}
			// 该标的是否是新手标,3-1
			...
			$borrow_investor_id = $this->db_invest($data, $investor_uid, $merOrderNo, $bid);
	...
	/** 投资 */
	private function db_invest($data, $investor_uid, $merOrderNo, $bid) {
		if(isset($data['money'])) {
			$this->db->where('uid', $investor_uid)->update('members_money', $data['money']);
	...
	public function upbind($body = array()) {
		...
		$bind = $this->bind_model->get_bind_byuid($water['uid']);
		$bind['status'] = 3;
		$bind['bank_card'] = $data['quick']['paycard'];
		$this->bind_model->modify_bind($bind);
	}
	...
	public function withdraw_tx($body = array(), $async) {
		...
		if($async) {//异步并且未处理过
			...
			if($body['head']['respCode'] == '000000' || $body['head']['respDesc'] == '处理中') {
				$members_money = $this->member_model->get_members_money_byuid($water['uid']);
				$money = round(floatval($water['money']), 2);
				$data['money']['account_money'] = $members_money['account_money'] - $money;
				//回写表members_money_log
				...
				//修改流水表状态为1
				$data['water']['status'] = 1;
				log_record('提现失败.原因'.$body['head']['respDesc'].'订单号为:'.$merOrderNo.'数据'.serialize($data), '', $water['uid'].'-82');
			}
			/*$members_money = $this->member_model->get_members_money_byuid($water['uid']);
			$money = round(floatval($water['money']), 2);
			//账户冻结一定金额
