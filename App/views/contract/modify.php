    ...
    <div class="main-wrap" style='padding:10px;'>
        ...
        <form action="/contract/add" class="layui-form layui-form-pane" id="formrec" method="post" role="form">
            ...
            <div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input name="name" autocomplete="off" lay-verify="required" value="<?php echo $contract['name']; ?>" placeholder="必填,合同名称" class="layui-input" type="text" />
					</div>
				</div>
				<div class="layui-inline">
					...
					<div class="layui-input-block" style="z-index:1000;">
						<select data-val="true" data-val-number="字段 Int32 必须是一个数字" data-val-required="Int32 字段是必需的" name="genre" />
							<option value="0"  <?php if(empty($contract['genre'])) { echo 'selected'; }  ?>>选择类型</option>
							<option value="服务合同" <?php if($contract['genre'] == '服务合同') { echo 'selected'; }  ?>>服务合同</option>
							<option value="投资合同" <?php if($contract['genre'] == '投资合同') { echo 'selected'; }  ?>>投资合同</option>
						</select>
					</div>
				</div>
				<div class="layui-inline">
					...
					<div class="layui-input-block" style="z-index:1000;">
						<select data-val="true" data-val-number="字段 Int32 必须是一个数字" data-val-required="Int32 字段是必需的" name="status" />
							<option selected="selected" value="-1" <?php if($contract['status'] === '') { echo 'selected'; }  ?>>选择类型</option>
							<option value="0" <?php if($contract['status'] === '0') { echo 'selected'; }  ?>>禁用</option>
							<option value="1" <?php if($contract['status'] === '1') { echo 'selected'; }  ?>>启用</option>
						</select>
					</div>
				</div>
            </div>
			<div class="layui-form-item">
				...
				<script id="editor" type="text/plain" style="width:100%;height:500px;"><?php echo $contract['mold']; ?></script>
            </div>
            <!--底部工具栏-->
                ...
                <div class="btn-list">
                    <div class="btnlist">
						<?php if(is_rule('/contract/modify')) { ?>
							<a class="layui-btn layui-btn-sm" lay-submit="" lay-filter="doPost" data-url="/contract/modify.html"><i class="layui-icon">&#x1005;</i>提交</a>
						<?php } ?>
