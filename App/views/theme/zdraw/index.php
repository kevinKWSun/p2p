...
<html>
	<head>
		...
		<link rel="stylesheet" href="/src/zdraw/css/maydraw.css" type="text/css" />
		...
		<div class="maydraw-main">
			<div class="drawing">
				...
				<div class="residue-num">
					<?php echo isset($zdraw) ? $zdraw['num'] : 0; ?>
				</div>
			...
			$(".immediately").click(function() {
				...
				if(login == 0) {
					...
					layer.confirm('请先登陆', {title:'登陆提示'}, function(index){
						location.href= '/suny/login.html?ret_url=/zdraw.html';
					});
				} else {
					var num = parseInt($('.residue-num').html());
					if(!num) {
						layer.confirm('剩余抽奖次数不足，获取抽奖次数', {title:'出借提示'}, function(index){
							location.href= '/invest.html';
						});
						zdraw.flag = true;
						$(".immediately").css('pointer-events',"auto");
					} else {
						var url = '/zdraw/random';
						$.post(url, function(r) {
							if(r.state == 1) {
								if(r.message) {
									$("#dataNums").empty();
