                        ...
                        <div class="l-list clearfix">
                            <form id="tt" class="layui-form layui-form-pane">
                                <div class="layui-form-item">
                                    ...
                                    <div class="layui-inline">
                                        <a class="layui-btn layui-btn-small" lay-submit="" lay-filter="js-search" data-url="/borrowing/audit.html">
                                            <i class="fa fa-search"></i>查询
                                        </a>
                                        <a class="layui-btn layui-btn-small do-action" data-type="doRefresh" data-url="/borrowing/audit.html">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
									...
									<?php } ?>
									<?php if(is_rule('/borrowing/confirm')) { ?>
										<a class="layui-btn layui-btn-sm" lay-submit="" lay-filter="js-audit" data-href='/borrowing/confirm/<?php echo $v['id']; ?>.html' target='_blank'>
											<i class="icon-edit  fa fa-search"></i>
											审　核
										</a>
									<?php } ?>
									<?php if(is_rule('/borrowing/back')) { ?>
										<a class="layui-btn layui-btn-sm" lay-submit="" lay-filter="js-back" data-href='/borrowing/back/<?php echo $v['id']; ?>.html' target='_blank'>
											<i class="icon-edit  fa fa-search"></i>
											退　回
										</a>
									<?php } ?>
									<?php if(is_rule('/borrowing/audit_modify')) { ?>
										<a class="layui-btn layui-btn-sm do-action" data-type="doEdit" data-url="/borrowing/audit_modify/<?php echo $v['id'], '/', $p; ?>.html"><i class="icon-edit fa fa-search"></i>编　辑</a>
									<?php } ?>
                                </td>
                            </tr>
                            <?php endforeach;} ?>
	...
	<script type="text/javascript">
		layui.use(['layer', 'form'], function(){
			...
			form.on('submit(js-audit)', function(data) {
				...
				layer.confirm('你确定要审核吗?', function() {
					layer.load(1);
					$.ajax({
						url 		: url,
						type		: 'post',
						...
						dataType	: 'json',
						beforeSend	: function () {
							$(data.elem).attr('disabled', true).html('审核中...');
						},
						...
						complete	: function() {
							$(data.elem).attr('disabled', false).html('<i class="icon-edit  fa fa-search"></i>审　核');
						},
						error 		: function(data) {
			...
			form.on('submit(js-back)', function(data) {
				var url = $(this).data('href');
				layer.prompt({
				  formType: 2,
				  value: '',
